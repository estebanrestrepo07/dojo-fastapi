...
def get_book_by_isbn(isbn: str, response: Response):
    ...
    if not book:
        ...
        return {"message": "No encontraddo", "status_code": 404}
    response.status_code = status.HTTP_200_OK
    return {"message": "Obtenido con exito", "status_code": 200, "body": book}
...
def create_book(book: BookEntity, response: Response):
    ...
    if not book_db:
        ...
        return {"message": "Creado con exito", "status_code": 201, "body": book_db}
...
def update_book(isbn: str ,book: BookEntityUpdate, response: Response):
    ...
    if not book_db:
        response.status_code = status.HTTP_404_NOT_FOUND
        return {"message": "El elemento no encontrado para actualizar", "status_code": 404}
    book_db = book_repository.update_book(isbn,book)
    response.status_code = status.HTTP_200_OK
    return {"message": "Actualizado con extio", "status_code": 200, "body": book_db}
...
def delete_book(isbn: str, response: Response):
    ...
    if not book_db:
        response.status_code = status.HTTP_404_NOT_FOUND
        return {"message": "No se pudo borrar", "status_code": 404}
    response.status_code = status.HTTP_200_OK
